...
* Copyright (C) 2024 Kishan Jadav https://kishanjadav.com
* Ported from Java to Swift in 2018 by Jonas Theslöf
* - Kishan: modified to use SystemRandomNumberGenerator instead of GameKit's random functions as they are unavailable on watchOS.
*
...
public enum SudokuDifficulty: String {
 ...
 * possibility.
 */
  private var possibilities: [Int] = Array(
    repeating: 0, count: POSSIBILITY_SIZE)
 ...
 * moves needed to solve the puzzle and doesn't contain information about
 * bad guesses.
 */
 ...
 */
  public func getDifficulty() -> SudokuDifficulty {
    if getGuessCount() > 0 { return SudokuDifficulty.EXPERT }
    if getBoxLineReductionCount() > 0 { return SudokuDifficulty.INTERMEDIATE }
    if getPointingPairTripleCount() > 0 { return SudokuDifficulty.INTERMEDIATE }
    if getHiddenPairCount() > 0 { return SudokuDifficulty.INTERMEDIATE }
    if getNakedPairCount() > 0 { return SudokuDifficulty.INTERMEDIATE }
    if getHiddenSingleCount() > 0 { return SudokuDifficulty.EASY }
    if getSingleCount() > 0 { return SudokuDifficulty.SIMPLE }
    return SudokuDifficulty.UNKNOWN
  }
 ...
 */
  ...
  private func shuffleRandomArrays() {
    QQWing.shuffleArray(&randomBoardArray)
    QQWing.shuffleArray(&randomPossibilityArray)
  }
  ...
  public func generatePuzzleSymmetry(_ sym: Symmetry) -> Bool {
    ...
    if symmetry == Symmetry.RANDOM {
      symmetry = QQWing.getRandomSymmetry()
    }
    ...
    for i in (0..<QQWing.BOARD_SIZE) {
      ...
      if puzzle[position] > 0 {
        ...
        switch symmetry {
        case .ROTATE90:
          positionsym2 = QQWing.rowColumnToCell(
            QQWing.ROW_COL_SEC_SIZE - 1 - QQWing.cellToColumn(position),
            QQWing.cellToRow(position))
          positionsym3 = QQWing.rowColumnToCell(
            QQWing.cellToColumn(position),
            QQWing.ROW_COL_SEC_SIZE - 1 - QQWing.cellToRow(position))
          fallthrough
        case .ROTATE180:
          positionsym1 = QQWing.rowColumnToCell(
            QQWing.ROW_COL_SEC_SIZE - 1 - QQWing.cellToRow(position),
            QQWing.ROW_COL_SEC_SIZE - 1 - QQWing.cellToColumn(position))
        case .MIRROR:
          positionsym1 = QQWing.rowColumnToCell(
 ...
 */
  ...
  private func rollbackRound(_ round: Int) {
    if logHistory || recordHistory {
      addHistoryItem(LogItem(round, LogType.ROLLBACK))
    }
    for i in (0..<QQWing.BOARD_SIZE) {
    ...
    for i in (0..<QQWing.ROW_COL_SEC_SIZE) {
      ...
      if possibilities[valPos] == 0 {
        if localGuessCount == guessNumber {
          ...
          if logHistory || recordHistory {
            addHistoryItem(LogItem(round, LogType.GUESS, value, position))
          }
          try? mark(position, round, value)
          return true
        }
    ...
    for valIndex in (0..<QQWing.ROW_COL_SEC_SIZE) {
      for col in (0..<QQWing.ROW_COL_SEC_SIZE) {
        ...
        if inOneBox && colBox != -1 {
          ...
          let secStart: Int = QQWing.cellToSectionStartCell(
            QQWing.rowColumnToCell(row, col))
          ...
          if doneSomething {
            if logHistory || recordHistory {
              addHistoryItem(
                LogItem(round, LogType.COLUMN_BOX, valIndex + 1, colStart))
            }
            return true
          }
        }
    ...
    for valIndex in (0..<QQWing.ROW_COL_SEC_SIZE) {
      for row in (0..<QQWing.ROW_COL_SEC_SIZE) {
        ...
        if inOneBox && rowBox != -1 {
          ...
          let secStart: Int = QQWing.cellToSectionStartCell(
            QQWing.rowColumnToCell(row, column))
          let secStartRow: Int = QQWing.cellToRow(secStart)
          let secStartCol: Int = QQWing.cellToColumn(secStart)
          for i in (0..<QQWing.GRID_SIZE) {
            for j in (0..<QQWing.GRID_SIZE) {
          ...
          if doneSomething {
            if logHistory || recordHistory {
              addHistoryItem(
                LogItem(round, LogType.ROW_BOX, valIndex + 1, rowStart))
            }
            return true
          }
        }
        ...
        if inOneRow && boxRow != -1 {
          ...
          if doneSomething {
            if logHistory || recordHistory {
              addHistoryItem(
                LogItem(
                  round, LogType.POINTING_PAIR_TRIPLE_ROW, valIndex + 1,
                  rowStart))
            }
            return true
          }
        ...
        if inOneCol && boxCol != -1 {
          ...
          if doneSomething {
            if logHistory || recordHistory {
              addHistoryItem(
                LogItem(
                  round, LogType.POINTING_PAIR_TRIPLE_COLUMN, valIndex + 1,
                  colStart))
            }
            return true
          }
  ...
  private func removePossibilitiesInOneFromTwo(
    _ position1: Int, _ position2: Int, _ round: Int
  ) -> Bool {
    var doneSomething: Bool = false
    for valIndex in (0..<QQWing.ROW_COL_SEC_SIZE) {
      let valPos1: Int = QQWing.getPossibilityIndex(valIndex, position1)
            ...
            if valCount2 == 2 && r1 == r3 && r2 == r4 {
              ...
              for valIndex3 in (0..<QQWing.ROW_COL_SEC_SIZE) {
                if valIndex3 != valIndex && valIndex3 != valIndex2 {
                  ...
                  let valPos1: Int = QQWing.getPossibilityIndex(
                    valIndex3, position1)
                  let valPos2: Int = QQWing.getPossibilityIndex(
                    valIndex3, position2)
              ...
              if doneSomething {
                if logHistory || recordHistory {
                  addHistoryItem(
                    LogItem(
                      round, LogType.HIDDEN_PAIR_COLUMN, valIndex + 1,
                      QQWing.rowColumnToCell(r1, column)))
                }
                return true
              }
            }
            ...
            if valCount2 == 2 && si1 == si3 && si2 == si4 {
              ...
              if doneSomething {
                if logHistory || recordHistory {
                  addHistoryItem(
                    LogItem(
                      round, LogType.HIDDEN_PAIR_SECTION, valIndex + 1,
                      QQWing.sectionToCell(section, si1)))
                }
                return true
              }
            }
            ...
            if valCount2 == 2 && c1 == c3 && c2 == c4 {
              ...
              if doneSomething {
                if logHistory || recordHistory {
                  addHistoryItem(
                    LogItem(
                      round, LogType.HIDDEN_PAIR_ROW, valIndex + 1,
                      QQWing.rowColumnToCell(row, c1)))
                }
                return true
              }
            }
            ...
            if possibilities2 == 2 && arePossibilitiesSame(position, position2)
            {
              if row == QQWing.cellToRow(position2) {
                ...
                if doneSomething {
                  if logHistory || recordHistory {
                    addHistoryItem(
                      LogItem(round, LogType.NAKED_PAIR_ROW, 0, position))
                  }
                  return true
                }
              }
              if column == QQWing.cellToColumn(position2) {
                ...
                if doneSomething {
                  if logHistory || recordHistory {
                    addHistoryItem(
                      LogItem(round, LogType.NAKED_PAIR_COLUMN, 0, position))
                  }
                  return true
                }
              }
              if section == QQWing.cellToSectionStartCell(position2) {
                ...
                for i in (0..<3) {
                  for j in (0..<3) {
                    let position3: Int =
                      secStart + i + (QQWing.ROW_COL_SEC_SIZE * j)
                    if position3 != position && position3 != position2
                      && removePossibilitiesInOneFromTwo(
                        position, position3, round)
                    {
                ...
                if doneSomething {
                  if logHistory || recordHistory {
                    addHistoryItem(
                      LogItem(round, LogType.NAKED_PAIR_SECTION, 0, position))
                  }
                  return true
                }
              }
    ...
    for row in (0..<QQWing.ROW_COL_SEC_SIZE) {
      for valIndex in (0..<QQWing.ROW_COL_SEC_SIZE) {
        ...
        if count == 1 {
          ...
          if logHistory || recordHistory {
            addHistoryItem(
              LogItem(round, LogType.HIDDEN_SINGLE_ROW, value, lastPosition))
          }
    ...
    for col in (0..<QQWing.ROW_COL_SEC_SIZE) {
      for valIndex in (0..<QQWing.ROW_COL_SEC_SIZE) {
        ...
        if count == 1 {
          ...
          if logHistory || recordHistory {
            addHistoryItem(
              LogItem(round, LogType.HIDDEN_SINGLE_COLUMN, value, lastPosition))
          }
          try? mark(lastPosition, round, value)
          return true
        }
    ...
    for sec in (0..<QQWing.ROW_COL_SEC_SIZE) {
      ...
      for valIndex in (0..<QQWing.ROW_COL_SEC_SIZE) {
        ...
        if count == 1 {
          ...
          if logHistory || recordHistory {
            addHistoryItem(
              LogItem(round, LogType.HIDDEN_SINGLE_SECTION, value, lastPosition)
            )
          }
          try? mark(lastPosition, round, value)
          return true
    ...
    for position in (0..<QQWing.BOARD_SIZE) {
      if solution[position] == 0 {
        ...
        if count == 1 {
          ...
          if logHistory || recordHistory {
            addHistoryItem(LogItem(round, LogType.SINGLE, lastValue, position))
          }
          return true
        }
      }
 ...
 */
  private func mark(_ position: Int, _ round: Int, _ value: Int) throws {
    if solution[position] != 0 {
      throw Errors.IllegalArgumentException(
        message: "Marking position that already has been marked.")
    }
    if solutionRound[position] != 0 {
      throw Errors.IllegalArgumentException(
        message: "Marking position that was marked another round.")
    }
    ...
    if possibilities[possInd] != 0 {
      throw Errors.IllegalArgumentException(
        message: "Marking impossible position.")
    }
 ...
 */
  private static func shuffleArray(_ array: inout [Int]) {
    for i in (0..<array.count) {
      let randTailPos: Int = Int.random(in: i..<array.count)
  ...
  private static func getRandomSymmetry() -> Symmetry {
    // not the first and last value which are NONE and RANDOM
    let values: [Symmetry] = [.FLIP, .MIRROR, .ROTATE90, .ROTATE180]
